...
#define min(x, y) ({                \
  ...
  #define max(x, y) ({                \
    ...
    // void PickNext(Entry *E, int *index, Rectangle G1, Rectangle G2, int n_G1, int n_G2, int* s1, int* s2, int size){ //RANDOM REMAINING ENTRY, DECIDEGROUP STAYS
    //   int i = 0;
    //   int candidate;
    //   float area1 = Rectangle_Area(G1);
    //   float area2 = Rectangle_Area(G2);
    ...
    void PickSeeds(Entry *E, int *indexes){
      //printf("values = %d\n",indexes[0]);
      int i = 1;
        float LHS_x = E[0].MBR.maxx;
        int LHS_c_x = 0;
        float HLS_x = E[0].MBR.minx;
        int HLS_c_x = 0;

        float LLS_x = E[0].MBR.minx;
        float HHS_x = E[0].MBR.maxx;

        float LHS_y = E[0].MBR.maxy;
        int LHS_c_y = 0;
        float HLS_y = E[0].MBR.miny;
        int HLS_c_y = 0;

        float LLS_y = E[0].MBR.miny;
        float HHS_y = E[0].MBR.maxy;
        while (i < indexes[0]){
          float candidate_LHS_x = E[i].MBR.maxx;
          float candidate_HLS_x = E[i].MBR.minx;

          float candidate_LHS_y = E[i].MBR.maxy;
          float candidate_HLS_y = E[i].MBR.miny;
          if (candidate_LHS_x < LHS_x){
            LHS_x = candidate_LHS_x;
            LHS_c_x = i;
          }
          if (candidate_HLS_x > HLS_x){
            HLS_x = candidate_HLS_x;
            HLS_c_x = i;
          }
          if (candidate_LHS_y < LHS_y){
            LHS_y = candidate_LHS_y;
            LHS_c_y = i;
          }
          if (candidate_HLS_y > HLS_y){
            HLS_y = candidate_HLS_y;
            HLS_c_y = i;
          }
          //
          float candidate_LLS_x = E[i].MBR.minx;
          float candidate_HHS_x = E[i].MBR.maxx;
          if (candidate_HHS_x > HHS_x){
            HHS_x = candidate_HHS_x;
          }
          if (candidate_LLS_x < LLS_x){
            LLS_x = candidate_LLS_x;
          }

          float candidate_LLS_y = E[i].MBR.miny;
          float candidate_HHS_y = E[i].MBR.maxy;
          if (candidate_HHS_y > HHS_y){
            HHS_y = candidate_HHS_y;
          }
          if (candidate_LLS_y < LLS_y){
            LLS_y = candidate_LLS_y;
          }

          i = i + 1;
        }
        float range_x = (HHS_x-LLS_x);
        float distance_x = (HLS_x-LHS_x)/range_x;
        //printf("HLS_x %.4f LHS_x %.4f diff %.4f\n",HLS_x,LHS_x,HLS_x-LHS_x);

        float range_y = (HHS_y-LLS_y);
        float distance_y = (HLS_y-LHS_y)/range_y;
        //printf("HLS_y %.4f LHS_y %.4f diff %.4f\n",HLS_y,LHS_y,HLS_y-LHS_y);

        //printf("distances %.4f %.4f\n", distance_x, distance_y);
        ...
        //printf("range x: %.4f range y: %.4f Distance x: %.4f Distance y: %.4f pair chosen %d %d\n",range_x,range_y,distance_x,distance_y,indexes[0],indexes[1]);
    }

    // void PickSeeds(Entry *E, int *indexes){ //HAS TO BECOME LINEARPICKSEEDS
    //
    ...
    void Linear_Split(Node *N, Entry E, Node *LL, Tree *T){ //HAS TO BECOME LINEAR_SPLIT
      ...
      //printf("rectangle to insert ");
      //print_rectangle(E.MBR);
      //printf(" to node %d\n", N->node_id);
    ...
    void Insert(Entry E, Tree* T){ //CHANGE QUAD SPLIT TO LINEAR SPLIT
    ...
    Rectangle random_rectangle(void){
      int x = random_int(100);
      while(x == 0){
        x = random_int(100);
      }
      float yy = random_float();

      int y = random_int(100);
      while(y == 0){
        y = random_int(100);
      }
    ...
    void insert_data_to_tree(Entry *EE, int num, int po){
      ...
      //print_tree(tree); //COMMENT THIS TO AVOID PRINTING TREE
      printf("\n|||||Tree of 2 to the %d|||||\n",po);
      ...
      printf("guaranteed avg reads %.4f || random avg reads %.4f\n", (float)total_guaranteed_reads/search, (float)total_random_reads/search);
